...
def test_candidate_and_canonical_document_counts_are_not_conflated():
    ...
    manifest = load(MANIFEST_PATH)
    candidates = {
        item["sha256"]: item["reviewed_row_count"]
        for item in source["documents"]
    }
    canonical = Counter(
        item["source_document_sha256"] for item in source["records"]
    )
    inventory_candidates = {
        item["sha256"]: item["reviewed_candidate_rows"]
        for item in manifest["source_document_inventory"]
    }
    inventory_canonical = {
        item["sha256"]: item["canonical_records"]
        for item in manifest["source_document_inventory"]
        if item["canonical_records"]
    }
...
def test_all_records_validate_and_preserve_sequence_and_source_fields():
    ...
    for source, record in zip(source_records, records, strict=True):
        ...
        assert actual["plan_current"]["value_text"] == source[
...
def test_dynamic_counts_and_zero_record_document_are_exact():
    ...
    assert Counter(
        item["source_location"]["location_kind"] for item in records
    ) == {"pdf_table_row": 1399, "pdf_text_line": 101}
    ...
    assert len({item["source_document_title"] for item in records}) == 2
    unused = [
        item
        for item in manifest["source_document_inventory"]
