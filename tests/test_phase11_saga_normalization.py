...
SCRIPT_PATH = ROOT / "scripts/normalize_phase11_saga.py"
SOURCE_PATH = ROOT / "data/reviewed/phase9/41.json"
...
MANIFEST_PATH = ROOT / "data/catalog/phase11_saga_normalization.json"
MANIFEST_SCHEMA_PATH = ROOT / "schemas/phase11_saga_normalization.schema.json"

spec = importlib.util.spec_from_file_location("normalize_phase11_saga", SCRIPT_PATH)
...
def test_manifest_source_and_summary_reconcile():
    ...
    summary = next(
        ...
        if item["prefecture_code"] == "41"
    )
    reviewed_rows = [item["reviewed_row_count"] for item in source["documents"]]
    assert len(source["records"]) == 435
    assert source["reviewed_target_statement_count"] == 435
    assert source["evidence_packet_count"] == 435
    assert len(source["documents"]) == 1
    assert source["landing_audit"]["landing_content_type"] == "application/pdf"
    assert reviewed_rows == [435]
    assert sum(reviewed_rows) == 435
    assert sum(count == 0 for count in reviewed_rows) == 0
    assert summary["reviewed_target_statement_count"] == 435
    assert summary["evidence_packet_count"] == 435
...
def test_all_records_validate_and_preserve_source_fields():
    ...
    assert [item["display_order"] for item in source_records] == list(range(1, 436))
    assert [item["subject"]["sequence"] for item in records] == list(range(1, 436))
...
def test_document_and_accountability_layers_remain_explicit():
    source = load(SOURCE_PATH)
    counts = Counter(item["source_document_sha256"] for item in source["records"])
    assert [counts[item["sha256"]] for item in source["documents"]] == [435]
    for record in catalog()["records"]:
        assert "登録された正本1件435行" in record["boundary"]
        assert "10年後の将来像" in record["boundary"]
        assert "年度別実施状況" in record["boundary"]
        assert "決算成果" in record["boundary"]
        assert "支出額を政策成果へ直接変換せず" in record["boundary"]
...
def test_summary_and_determinism_are_exact():
    ...
    assert normalized["summary"] == {
        ...
        "partial_record_count": 435,
        "not_linked_record_count": 0,
        "indicator_series_count": 435,
        ...
        "reviewed_maximum_depth_record_count": 435,
