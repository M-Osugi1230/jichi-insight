...
def test_chiba_field04_reposts_reconcile_without_identity_inflation():
    ...
    linked = next(row for row in cross if row["project_name"] == "発達障害支援の推進")
    assert linked["primary_review_id"] == "chiba-f03-p016"


def test_chiba_field04_resolves_prior_cross_field_primary_locations():
    field04_names = {row["project_name"] for row in load(FIELD04)["records"]}
    expected_primary_names = {
        "おたふくかぜ予防接種費用の助成",
        "ＲＳウイルス母子免疫ワクチン予防接種の実施",
        "新東部児童相談所及び発達に係る相談支援機関等の整備",
        "特別支援教育の充実",
        "市立学校の体育館冷暖房設備の整備",
    }

    assert expected_primary_names.issubset(field04_names)
...
def test_chiba_manifest_retains_field04_completion_as_review_advances():
    ...
    assert (
        "data/catalog/chiba_current_project_identities_field04.json"
    ...
    assert fact["reviewed_field_counts"]["children_education"] == 34
