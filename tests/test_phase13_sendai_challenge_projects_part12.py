...
DATA_PATH = ROOT / "data/catalog/sendai_challenge_project_reviews_part12.json"
EVIDENCE_PATH = ROOT / "data/evidence/sendai_challenge_project_reviews_part12_evidence.json"
...
def test_sendai_challenge_part12_counts_and_coverage_are_exact():
    data = load(DATA_PATH)
    summary = data["summary"]

    assert summary["prior_reviewed_record_count"] == 33
    assert summary["batch_reviewed_record_count"] == 3
    assert summary["cumulative_reviewed_record_count"] == 36
    assert summary["total_source_project_count"] == 108
    assert summary["remaining_record_count"] == 72
    assert summary["complete"] is False


def test_sendai_challenge_part12_records_are_exact_and_source_reported():
    ...
    assert [record["lead_section_ja"] for record in records] == [
        "こども若者局　いじめ対策推進課／教育局　教育相談課",
        "こども家庭保健課",
        "若者支援課",
    ]


def test_sendai_challenge_part12_evidence_is_one_to_one_and_valid():
    data = load(DATA_PATH)
...
def test_sendai_part12_preserves_multi_bureau_and_causality_boundaries():
    data = load(DATA_PATH)
    evidence = load(EVIDENCE_PATH)
    bullying = data["records"][0]
    packet = next(packet for packet in evidence if packet["subject_id"] == bullying["id"])

    assert bullying["responsible_bureau_ja"] == "こども若者局・教育局"
    assert "いじめ対策推進課" in bullying["lead_section_ja"]
    assert "教育相談課" in bullying["lead_section_ja"]
    assert "因果評価" in packet["claims"][0]["review_note"]
    assert "causal claim" in data["quality_boundary"]


def test_sendai_manifest_advances_to_thirty_six_without_claiming_completion():
    manifest = load(MANIFEST_PATH)
    facts = {fact["id"]: fact for fact in manifest["reviewed_facts"]}
    part12 = facts["sendai-challenge-project-records-part12"]
    batches = [
        fact
    ...
    assert part12["value"] == 3
    assert part12["cumulative_value"] == 36
    assert part12["source_reported_breakdown"] == {"circle": 3}
    assert "残り72事業" in part12["interpretation_boundary"]
    assert cumulative_reviewed >= 36
