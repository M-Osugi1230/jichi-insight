...
SCRIPT_PATH = ROOT / "scripts/normalize_phase11_wakayama.py"
SOURCE_PATH = ROOT / "data/reviewed/phase9/30.json"
...
MANIFEST_PATH = ROOT / "data/catalog/phase11_wakayama_normalization.json"
MANIFEST_SCHEMA_PATH = ROOT / "schemas/phase11_wakayama_normalization.schema.json"

spec = importlib.util.spec_from_file_location("normalize_phase11_wakayama", SCRIPT_PATH)
...
def test_manifest_source_and_phase9_summary_reconcile():
    ...
    summary = next(
        ...
        if item["prefecture_code"] == "30"
    )
    reviewed_rows = [item["reviewed_row_count"] for item in source["documents"]]
    assert len(source["records"]) == 35
    assert source["reviewed_target_statement_count"] == 35
    assert source["evidence_packet_count"] == 35
    assert len(source["documents"]) == 6
    assert source["landing_audit"]["selected_document_count"] == 6
    assert reviewed_rows == [35, 0, 0, 33, 0, 0]
    assert sum(reviewed_rows) == 68
    assert sum(count == 0 for count in reviewed_rows) == 4
    assert summary["reviewed_target_statement_count"] == 35
    assert summary["evidence_packet_count"] == 35
    assert summary["document_count"] == 6
    assert summary["extraction_error_count"] == 0


def test_all_35_records_validate_and_preserve_sequence():
    source_records = load(SOURCE_PATH)["records"]
    records = catalog()["records"]
    validator = Draft202012Validator(
        load(RECORD_SCHEMA_PATH), format_checker=FormatChecker()
    )
    assert [item["display_order"] for item in source_records] == list(range(1, 36))
    assert [item["subject"]["sequence"] for item in records] == list(range(1, 36))
    assert [item["source_record_id"] for item in records] == [
        item["id"] for item in source_records
    ]
    assert len({item["id"] for item in records}) == 35
...
def test_overlap_and_zero_row_documents_remain_explicit():
    source = load(SOURCE_PATH)
    used_hashes = {item["source_document_sha256"] for item in source["records"]}
    document_counts = {item["sha256"]: item["reviewed_row_count"] for item in source["documents"]}
    assert len(document_counts) == 6
    assert len(used_hashes) == 1
    assert next(iter(used_hashes)) == source["documents"][0]["sha256"]
    assert source["documents"][3]["reviewed_row_count"] == 33
    assert source["documents"][3]["sha256"] not in used_hashes
    assert sum(count == 0 for count in document_counts.values()) == 4
    for record in catalog()["records"]:
        assert "35行とアクションプラン版33行" in record["boundary"]
...
def test_records_remain_partial_unassessed_and_noncomparable():
    for record in catalog()["records"]:
        ...
        assert "2030年度・2040年度目標" in record["boundary"]
        assert "初年度評価が未公表" in record["boundary"]


def test_dynamic_counts_and_determinism_are_exact():
    source_records = load(SOURCE_PATH)["records"]
    normalized = catalog()
    ...
    assert normalized["summary"] == {
        ...
        "partial_record_count": 35,
        "not_linked_record_count": 0,
        "indicator_series_count": 35,
        ...
        "reviewed_maximum_depth_record_count": 35,
