...
def test_work_packages_and_quality_boundaries():
    ...
    assert "系列1〜40" in packages["kpi_catalog"]["deliverable"]
    assert "残る90目標グループ" in packages["kpi_catalog"]["deliverable"]
    assert "109系列" in packages["kpi_catalog"]["deliverable"]
    assert packages["evaluation_linkage"]["status"] == "queued"
    assert packages["web_publication"]["status"] == "blocked"
    assert "目標グループ39〜52" in manifest["open_questions"][0]
    assert "系列41〜57" in manifest["open_questions"][0]
