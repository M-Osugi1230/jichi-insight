...
def test_manifest_schema_counts_and_progress():
    ...
    assert manifest["reviewed_target_group_count"] == 100
    assert manifest["reviewed_indicator_series_count"] == 119
    assert manifest["remaining_target_group_count"] == 28
    assert manifest["remaining_indicator_series_count"] == 30
    assert manifest["kpi_evidence_packet_count"] == 100
    assert 100 + 28 == 128
    assert 119 + 30 == 149
...
def test_work_packages_and_quality_boundaries():
    ...
    assert "目標グループ1〜100" in packages["kpi_catalog"]["deliverable"]
    assert "残る28目標グループ" in packages["kpi_catalog"]["deliverable"]
    ...
    assert "目標グループ101〜104" in manifest["open_questions"][0]
    assert "累計1系列" in manifest["open_questions"][1]
    assert "3939・2428" in manifest["open_questions"][1]
