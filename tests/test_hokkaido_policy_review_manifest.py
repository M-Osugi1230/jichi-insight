...
def test_manifest_counts_and_boundaries():
    ...
    assert manifest["reviewed_indicator_count"] == 98
    assert manifest["remaining_indicator_count"] == 10
    assert manifest["indicator_evidence_packet_count"] == 98
    assert (
...
def test_work_packages_record_sixteen_reviewed_fields():
    ...
    assert "指標1〜98" in packages["indicator_catalog"]["deliverable"]
    assert "指標99〜108" in packages["indicator_catalog"]["deliverable"]
    assert "指標1〜98" in packages["evidence_packets"]["deliverable"]
    assert "残る10指標" in packages["evidence_packets"]["deliverable"]
