...
def test_hokkaido_manifest_preserves_boundaries_and_partial_counts():
    ...
    assert manifest["reviewed_indicator_count"] == 39
    assert manifest["remaining_indicator_count"] == 69
    assert manifest["indicator_evidence_packet_count"] == 39
    assert (
...
def test_hokkaido_work_packages_record_five_reviewed_fields():
    ...
    assert "指標1〜39" in packages["indicator_catalog"]["deliverable"]
    assert "指標40〜108" in packages["indicator_catalog"]["deliverable"]
    assert packages["evidence_packets"]["status"] == "active"
    assert "指標1〜39" in packages["evidence_packets"]["deliverable"]
    assert "残る69指標" in packages["evidence_packets"]["deliverable"]
