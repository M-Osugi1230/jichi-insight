...
def test_hokkaido_manifest_preserves_boundaries_and_partial_counts():
    ...
    assert manifest["reviewed_indicator_count"] == 70
    assert manifest["remaining_indicator_count"] == 38
    assert manifest["indicator_evidence_packet_count"] == 70
    assert (
...
def test_hokkaido_work_packages_record_ten_reviewed_fields():
    ...
    assert "指標1〜70" in packages["indicator_catalog"]["deliverable"]
    assert "指標71〜108" in packages["indicator_catalog"]["deliverable"]
    assert packages["evidence_packets"]["status"] == "active"
    assert "指標1〜70" in packages["evidence_packets"]["deliverable"]
    assert "残る38指標" in packages["evidence_packets"]["deliverable"]
