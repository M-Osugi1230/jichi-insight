...
def test_manifest_counts_and_boundaries():
    ...
    assert manifest["reviewed_indicator_count"] == 79
    assert manifest["remaining_indicator_count"] == 29
    assert manifest["indicator_evidence_packet_count"] == 79
    assert (
...
def test_work_packages_record_twelve_reviewed_fields():
    ...
    assert "指標1〜79" in packages["indicator_catalog"]["deliverable"]
    assert "指標80〜108" in packages["indicator_catalog"]["deliverable"]
    assert "指標1〜79" in packages["evidence_packets"]["deliverable"]
    assert "残る29指標" in packages["evidence_packets"]["deliverable"]
