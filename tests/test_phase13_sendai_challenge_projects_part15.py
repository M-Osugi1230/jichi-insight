...
DATA_PATH = ROOT / "data/catalog/sendai_challenge_project_reviews_part15.json"
EVIDENCE_PATH = ROOT / "data/evidence/sendai_challenge_project_reviews_part15_evidence.json"
...
def test_sendai_challenge_part15_counts_and_coverage_are_exact():
    data = load(DATA_PATH)
    summary = data["summary"]

    assert summary["prior_reviewed_record_count"] == 42
    assert summary["batch_reviewed_record_count"] == 3
    assert summary["cumulative_reviewed_record_count"] == 45
    assert summary["total_source_project_count"] == 108
    assert summary["remaining_record_count"] == 63
    assert summary["complete"] is False


def test_sendai_challenge_part15_records_are_exact_and_source_reported():
    records = load(DATA_PATH)["records"]
    assert len(records) == 3
    assert [record["project_name_ja"] for record in records] == [
        "「Fun,Fan,Find青葉」事業",
        "先端技術等を活用した宮城地区の課題解決事業",
        "長町・歩いて楽しい街並み形成促進事業",
    ]
    assert [record["source_pdf_page_index_0_based"] for record in records] == [53, 54, 54]
...
def test_sendai_challenge_part15_evidence_is_one_to_one_and_valid():
    data = load(DATA_PATH)
...
def test_sendai_part15_preserves_causality_boundaries():
    data = load(DATA_PATH)
...
def test_sendai_manifest_advances_to_forty_five_without_claiming_completion():
    manifest = load(MANIFEST_PATH)
    facts = {fact["id"]: fact for fact in manifest["reviewed_facts"]}
    part15 = facts["sendai-challenge-project-records-part15"]
    batches = [
        fact
    ...
    assert part15["value"] == 3
    assert part15["cumulative_value"] == 45
    assert part15["source_reported_breakdown"] == {"circle": 3}
    assert "残り63事業" in part15["interpretation_boundary"]
    assert cumulative_reviewed >= 45
