...
def test_reviewed_groups_series_and_evidence_are_contiguous():
    ...
    assert [entry["series_number"] for entry in series] == list(range(1, 113))
    assert len({group["id"] for group in groups}) == 93
    assert len({entry["id"] for entry in series}) == 112
    assert len(packets) == 93
    assert len({packet["id"] for packet in packets}) == 93
...
def test_manifest_matches_reviewed_files():
    ...
    series = [entry for group in groups for entry in group["series"]]
    packets = [packet for path in EVIDENCE for packet in load(path)]
    assert manifest["reviewed_target_group_count"] == len(groups) == 93
    assert manifest["reviewed_indicator_series_count"] == len(series) == 112
    assert manifest["kpi_evidence_packet_count"] == len(packets) == 93
