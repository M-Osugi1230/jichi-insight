...
def test_catalog_schema_sequence_and_pages():
    ...
    assert (data["indicator_number_start"], data["indicator_number_end"]) == (103, 108)
    assert [item["indicator_number"] for item in data["items"]] == list(range(103, 109))
...
def test_catalog_matches_verified_page_index():
    ...
    index = {
        ...
        for item in load(ROOT / "data/catalog/hokkaido_indicator_page_index.json")["records"]
    }
...
def test_exact_names_values_and_periods_are_preserved():
    ...
    assert [items[number]["indicator_name_original"] for number in range(103, 109)] == [
        "北海道博物館の利用者数",
        "文化会館１館当たりの年間入館者数",
        "アイヌ民族が先住民族であることの認知度",
        "成人の週１回以上スポーツ実施率",
    ...
    assert [[value["value"] for value in items[number]["series"][0]["values"]] for number in range(103, 109)] == [
    ...
    assert [value["period"] for value in items[106]["series"][0]["values"]] == [
        "2021年", "2027年", "2031年"
    ]
    assert [value["period"] for value in items[107]["series"][0]["values"]] == [
...
def test_past_high_targets_remain_conditional():
    items = {item["indicator_number"]: item for item in load(CATALOG)["items"]}
    for number, reference_text in [(107, "過去最高値60人"), (108, "過去最高値8個")]:
...
def test_reference_breakdowns_and_publication_dates_are_excluded():
    ...
    assert "オリンピック・パラリンピック別内訳" in items[108]["comparability_note_original"]
