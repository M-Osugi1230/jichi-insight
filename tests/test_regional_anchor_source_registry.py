...
def test_all_nine_anchors_are_reviewed_with_repository_evidence():
    records = load(REGISTRY_PATH)["records"]
    assert all(record["anchor_status"] == "reviewed_reference" for record in records)
    assert all(record["numeric_target_status"] == "reviewed" for record in records)
...
def test_six_source_discovery_anchors_keep_all_official_source_categories():
    registry = load(REGISTRY_PATH)
    categories = set(registry["source_categories"])
    source_mapped_codes = {"13", "23", "27", "34", "37", "47"}
    by_code = {record["prefecture_code"]: record for record in registry["records"]}

    for code in source_mapped_codes:
        record = by_code[code]
    ...
    for code in {"01", "04", "40"}:
        assert by_code[code]["sources"] == []
