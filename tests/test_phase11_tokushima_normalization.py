...
SCRIPT_PATH = ROOT / "scripts/normalize_phase11_tokushima.py"
SOURCE_PATH = ROOT / "data/reviewed/phase9/36.json"
...
MANIFEST_PATH = ROOT / "data/catalog/phase11_tokushima_normalization.json"
MANIFEST_SCHEMA_PATH = ROOT / "schemas/phase11_tokushima_normalization.schema.json"

spec = importlib.util.spec_from_file_location("normalize_phase11_tokushima", SCRIPT_PATH)
...
def test_manifest_source_and_summary_reconcile():
    ...
    summary = next(
        ...
        if item["prefecture_code"] == "36"
    )
    reviewed_rows = [item["reviewed_row_count"] for item in source["documents"]]
    assert len(source["records"]) == 426
    assert source["reviewed_target_statement_count"] == 426
    assert source["evidence_packet_count"] == 426
    assert len(source["documents"]) == 5
    assert source["landing_audit"]["selected_document_count"] == 6
    assert reviewed_rows == [0, 132, 112, 179, 3]
    assert sum(reviewed_rows) == 426
...
def test_all_records_validate_and_preserve_source_fields():
    ...
    assert [item["display_order"] for item in source_records] == list(range(1, 427))
    assert [item["subject"]["sequence"] for item in records] == list(range(1, 427))
...
def test_document_layers_and_version_boundaries_remain_explicit():
    ...
    assert [counts[item["sha256"]] for item in source["documents"]] == [
        0, 132, 112, 179, 3
    ]
    for record in catalog()["records"]:
        assert "令和7年3月概要版132行" in record["boundary"]
        assert "KPI担当一覧112行" in record["boundary"]
        assert "令和7年3月全体版179行" in record["boundary"]
        assert "具体的取組工程担当一覧3行" in record["boundary"]
        assert "令和6年3月概要版の0行状態" in record["boundary"]
...
def test_summary_and_determinism_are_exact():
    ...
    assert normalized["summary"] == {
        ...
        "partial_record_count": 426,
        "not_linked_record_count": 0,
        "indicator_series_count": 426,
        ...
        "reviewed_maximum_depth_record_count": 426,
