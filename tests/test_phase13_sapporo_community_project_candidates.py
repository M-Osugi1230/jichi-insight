...
def test_community_historical_candidate_inventory_has_exact_47_rows():
    ...
    assert Counter(record["record_type"] for record in records) == {"main_project": 27, "other_project": 20}
...
def test_community_candidate_inventory_preserves_representative_anchors():
    ...
    assert records["future_smile_city_activity_promotion"]["planned_project_cost_yen"] == 1_855_000_000
...
def test_community_historical_candidates_have_one_to_one_evidence():
    ...
    assert {packet["candidate_id"] for packet in packets} == {record["id"] for record in catalog["records"]}
...
def test_community_final_review_promotes_all_47_rows_after_direct_visual_check():
    ...
    community_denominator = next(field for field in denominators["field_denominators"] if field["field_id"] == "community")
...
def test_community_final_review_is_reflected_in_central_metadata():
    ...
    community = next(field for field in index["machizukuri_field_sources"] if field["field_id"] == "community")
    ...
    assert sources["sapporo-action-plan-2023-projects-community"]["reviewed_project_record_count"] == 47
