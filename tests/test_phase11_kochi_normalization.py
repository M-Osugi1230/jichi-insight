...
SCRIPT_PATH = ROOT / "scripts/normalize_phase11_kochi.py"
SOURCE_PATH = ROOT / "data/reviewed/phase9/39.json"
...
MANIFEST_PATH = ROOT / "data/catalog/phase11_kochi_normalization.json"
MANIFEST_SCHEMA_PATH = ROOT / "schemas/phase11_kochi_normalization.schema.json"

spec = importlib.util.spec_from_file_location("normalize_phase11_kochi", SCRIPT_PATH)
...
def test_manifest_source_and_summary_reconcile():
    ...
    summary = next(
        ...
        if item["prefecture_code"] == "39"
    )
    reviewed_rows = [item["reviewed_row_count"] for item in source["documents"]]
    assert len(source["records"]) == 270
    assert source["reviewed_target_statement_count"] == 270
    assert source["evidence_packet_count"] == 270
    assert len(source["documents"]) == 1
    assert source["landing_audit"]["selected_document_count"] == 2
    assert reviewed_rows == [270]
    assert sum(reviewed_rows) == 270
    assert sum(count == 0 for count in reviewed_rows) == 0
    assert summary["reviewed_target_statement_count"] == 270
    assert summary["evidence_packet_count"] == 270
...
def test_all_records_validate_and_preserve_source_fields():
    ...
    assert [item["display_order"] for item in source_records] == list(range(1, 271))
    assert [item["subject"]["sequence"] for item in records] == list(range(1, 271))
...
def test_document_and_version_boundaries_remain_explicit():
    ...
    assert [counts[item["sha256"]] for item in source["documents"]] == [270]
    for record in catalog()["records"]:
        assert "登録正本PDF1件の270行" in record["boundary"]
        assert "令和6・7年度版から推測で行を追加しない" in record["boundary"]
        assert "戦略全体目標" in record["boundary"]
        assert "年度実績" in record["boundary"]
        assert "産業振興計画等の個別計画指標" in record["boundary"]
...
def test_summary_and_determinism_are_exact():
    ...
    assert normalized["summary"] == {
        ...
        "partial_record_count": 270,
        "not_linked_record_count": 0,
        "indicator_series_count": 270,
        ...
        "reviewed_maximum_depth_record_count": 270,
