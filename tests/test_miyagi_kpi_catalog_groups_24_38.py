...
def test_multi_series_and_original_format():
    items = {item["target_group_number"]: item for item in groups()}
    group = items[26]
    assert [entry["series_number"] for entry in group["series"]] == [26, 27, 28]
    assert [[value["value"] for value in entry["values"]] for entry in group["series"]] == [
        [131, 154, 160, None],
        [31, 24, 54, None],
        [33, 69, 100, None],
    ]
    assert [value["period_original"] for value in group["series"][2]["values"][:2]] == [
        "R4",
        "R5",
    ]
    assert "水産業 ）（人）" in group["series"][1]["indicator_name_original"]
...
def test_missing_targets_and_review_boundaries():
    items = {item["target_group_number"]: item for item in groups()}
...
def test_evidence_covers_each_group_once():
