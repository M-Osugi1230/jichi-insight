...
def test_chiba_plan_review_reflects_field04_visual_completion():
    ...
    assert work_items["structured_project_count"] == 156
    assert work_items["pending_visual_column_confirmation_project_count"] == 33
    assert work_items["structured_work_item_count"] == 339
    assert "33" in review["next_action"]
