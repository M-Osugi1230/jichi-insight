...
def quantitative_rows(payload):
    return [
        ...
        for indicator in field["quantitative_indicators"]
    ]
...
def qualitative_rows(payload):
    return [
        factor
        for field in payload["fields"]
        for factor in field["qualitative_constituent_factors"]
    ]
...
def test_chiba_plan_review_reflects_field05_visual_completion():
    ...
    assert work_items["structured_project_count"] == 160
    assert work_items["pending_visual_column_confirmation_project_count"] == 29
    assert work_items["structured_work_item_count"] == 347
    assert "29" in review["next_action"]
