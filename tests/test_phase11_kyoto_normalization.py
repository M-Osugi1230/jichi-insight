...
def test_registered_documents_and_canonical_counts_are_exact():
    ...
    inventory = {
        item["sha256"]: item for item in manifest["source_document_inventory"]
    }
    ...
    assert (
        sum(item["reviewed_candidate_rows"] for item in inventory.values())
        == 282
    )
    assert (
        sum(item["canonical_status"] == "used" for item in inventory.values())
        == 3
    )
