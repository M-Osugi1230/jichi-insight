...
def test_manifest_schema_and_counts():
    ...
    assert manifest["actual_linked_target_group_count"] == 18
    assert manifest["actual_linked_indicator_series_count"] == 18
    assert manifest["actual_linkage_review_needed_series_count"] == 2
    assert manifest["actual_result_row_count"] == 80
    assert manifest["actual_evidence_packet_count"] == 20
