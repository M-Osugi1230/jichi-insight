...
def test_manifest_schema_and_counts():
    ...
    assert manifest["actual_linked_target_group_count"] == 51
    assert manifest["actual_linked_indicator_series_count"] == 63
    assert manifest["actual_linkage_review_needed_series_count"] == 10
    assert manifest["actual_result_row_count"] == 292
    assert manifest["actual_evidence_packet_count"] == 73
