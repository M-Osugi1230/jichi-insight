...
def reviewed_groups():
    return [group for path in CATALOG_PATHS for group in load(path)["items"]]


def evidence_packets():
    return [packet for path in EVIDENCE_PATHS for packet in load(path)]
...
def test_reviewed_catalogs_cover_series_1_to_40_without_gaps_or_duplicates():
    series = [item for group in reviewed_groups() for item in group["series"]]
