...
EVIDENCE = EVD / "saitama_current_project_target_identities_chapter08_evidence.json"
...
def test_saitama_chapter08_target_identity_review_covers_all_7_projects():
    official_ids = {
        row["project_code"]
        for row in load(IDENTITIES)["records"]
        if row["project_code"].startswith("08-")
    }
    reviewed = {row["project_code"] for row in load(CH8)["records"]}

    assert len(official_ids) == 7
    assert len(reviewed) == 7
    assert reviewed == official_ids


def test_saitama_chapter08_has_exactly_18_named_target_identities():
    rows = load(CH8)["records"]
    identities = [
    ...
    assert load(CH8)["target_indicator_identity_count"] == 18


def test_saitama_chapter08_measure_partition_reconciles_to_18():
    rows = load(CH8)["records"]
    expected = {"08-1-1": 11, "08-1-2": 4, "08-1-3": 3}
    actual = {
        prefix: sum(
...
def test_saitama_chapter08_remains_identity_only_until_values_are_reviewed():
    payload = load(CH8)
    value_fields = {
        "baseline",
        "annual_targets",
...
def test_saitama_target_identity_progress_reaches_123_projects_and_258_targets():
    ch1 = load(CH1)
    ...
    assert projects == 123
    assert targets == 258
    assert 258 - projects == 135
    assert 258 - ch1["project_count"] == 246


def test_saitama_chapter08_departments_locators_and_evidence_are_explicit():
    rows = load(CH8)["records"]
    ...
    assert evidence["review_status"] == (
        "reviewed_7_projects_18_target_identities_values_pending"
    )
    assert sum(row["project_count"] for row in evidence["evidence"]) == 7
    assert sum(row["target_identity_count"] for row in evidence["evidence"]) == 18
