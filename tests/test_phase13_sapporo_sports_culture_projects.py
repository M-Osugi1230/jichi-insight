...
def test_sports_culture_reconciliation_directly_confirms_excluded_olympic_candidate():
    ...
    assert (
        excluded["final_inclusion_status"]
        == "directly_confirmed_absent_from_final_printed_page_104"
    )
    ...
    assert (
        reconciliation["reconciliation"]["final_page104_observed_rows"][
            "winter_olympic_paralympic_related_present"
        ]
        is False
    )
...
def test_sports_culture_keeps_representative_candidate_numeric_anchors():
    ...
    assert (
        records["athlete_discovery_development_utilization"]["planned_project_cost_yen"]
        == 292_000_000
    )
