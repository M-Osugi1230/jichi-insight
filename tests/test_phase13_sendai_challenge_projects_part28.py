...
DATA_PATH = ROOT / "data/catalog/sendai_challenge_project_reviews_part28.json"
EVIDENCE_PATH = ROOT / "data/evidence/sendai_challenge_project_reviews_part28_evidence.json"
...
def test_sendai_challenge_part28_counts_and_coverage_are_exact():
    summary = load(DATA_PATH)["summary"]
    assert summary == {
        "prior_reviewed_record_count": 81,
        "batch_reviewed_record_count": 3,
        "cumulative_reviewed_record_count": 84,
        "total_source_project_count": 108,
        "remaining_record_count": 24,
        "complete": False,
    }


def test_sendai_challenge_part28_records_are_exact_and_source_reported():
    ...
    assert [record["source_pdf_page_index_0_based"] for record in records] == [98, 99, 100]
...
def test_sendai_challenge_part28_evidence_is_one_to_one_and_valid():
    data = load(DATA_PATH)
...
def test_sendai_part28_preserves_causality_boundaries():
    data = load(DATA_PATH)
