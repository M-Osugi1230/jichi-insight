...
SCRIPT_PATH = ROOT / "scripts/normalize_phase11_fukui.py"
SOURCE_PATH = ROOT / "data/reviewed/phase9/18.json"
...
MANIFEST_PATH = ROOT / "data/catalog/phase11_fukui_normalization.json"
MANIFEST_SCHEMA_PATH = ROOT / "schemas/phase11_fukui_normalization.schema.json"

spec = importlib.util.spec_from_file_location("normalize_phase11_fukui", SCRIPT_PATH)
...
def test_manifest_source_and_phase9_summary_reconcile():
    ...
    summary = next(
        ...
        if item["prefecture_code"] == "18"
    )
    reviewed_rows = [item["reviewed_row_count"] for item in source["documents"]]
    assert len(source["records"]) == 114
    assert source["reviewed_target_statement_count"] == 114
    assert source["evidence_packet_count"] == 114
    assert len(source["documents"]) == 6
    assert source["landing_audit"]["selected_document_count"] == 6
    assert reviewed_rows == [1, 102, 0, 7, 3, 1]
    assert sum(reviewed_rows) == 114
    assert sum(count == 0 for count in reviewed_rows) == 1
    assert summary["reviewed_target_statement_count"] == 114
    assert summary["evidence_packet_count"] == 114
    ...
    assert manifest["document_boundary"] == {
        ...
        "document_reviewed_row_counts": [1, 102, 0, 7, 3, 1],
        "document_reviewed_row_total": 114,
        "canonical_record_count": 114,
        "documents_with_zero_reviewed_rows": 1,
    }


def test_all_114_records_validate_and_preserve_sequence():
    source_records = load(SOURCE_PATH)["records"]
    records = catalog()["records"]
    validator = Draft202012Validator(
        load(RECORD_SCHEMA_PATH), format_checker=FormatChecker()
    )
    assert [item["display_order"] for item in source_records] == list(range(1, 115))
    assert [item["subject"]["sequence"] for item in records] == list(range(1, 115))
    assert [item["source_record_id"] for item in records] == [
        item["id"] for item in source_records
    ]
    assert len({item["id"] for item in records}) == 114
...
def test_document_roles_and_zero_row_document_remain_explicit():
    ...
    for record in catalog()["records"]:
        assert "1・102・0・7・3・1" in record["boundary"]
        assert "0行の全文版" in record["boundary"]


def test_vision_execution_region_and_evaluation_boundaries_are_preserved():
    source_text = " ".join(
        item["target_statement_original"] for item in load(SOURCE_PATH)["records"]
    )
    assert "2040" in source_text
    assert "実行プラン" in source_text
    assert "地域プラン" in source_text
    for record in catalog()["records"]:
        assert "2040年" in record["boundary"]
        assert "2025〜2029年度" in record["boundary"]
        assert "3つの広域区分" in record["boundary"]
        assert "6地域区分" in record["boundary"]
        assert "チャレンジ目標と結果" in record["boundary"]
...
def test_dynamic_document_location_and_unit_counts_are_exact():
    ...
    assert normalized["summary"] == {
        ...
        "partial_record_count": 114,
        "not_linked_record_count": 0,
        "indicator_series_count": 114,
        ...
        "reviewed_maximum_depth_record_count": 114,
