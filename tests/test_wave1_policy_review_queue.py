...
def test_reviewed_references_and_active_miyagi_progress():
    ...
    assert "目標グループ39〜52・系列41〜57" in miyagi["next_action"]
    assert "複数系列グループ26" in miyagi["priority_basis"]
    assert "現況値より低い公式目標" in miyagi["priority_basis"]
    assert "単位記載なし" in miyagi["priority_basis"]
    assert "桁区切りなし" in miyagi["priority_basis"]
