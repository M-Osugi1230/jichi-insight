...
def test_queue_covers_all_wave_one_anchors_once_in_operational_order():
    ...
    assert {item["prefecture_code"] for item in items} == set(coverage["regional_anchor_codes"])
...
def test_fukuoka_is_the_only_reviewed_reference_and_hokkaido_is_active():
    ...
    assert items["01"]["source_inventory_status"] == "indicator_relationships_reviewed"
    assert items["01"]["next_gate"] == "kpi_catalog"
    assert "指標1〜29" in items["01"]["next_action"]
    assert "Evidence Packet付き" in items["01"]["next_action"]
    assert "残る79指標" in items["01"]["next_action"]
    assert "指標30〜32（デジタル）" in items["01"]["next_action"]
...
def test_queue_sources_exist_and_titles_match_the_policy_source_catalog():
    queue = load(QUEUE_PATH)
    sources = {item["id"]: item for item in load(ROOT / "data/catalog/policy_sources.json")["records"]}
