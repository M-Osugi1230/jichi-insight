...
PARTS = range(30, 37)
EXPECTED_CUMULATIVE = {30: 90, 31: 93, 32: 96, 33: 99, 34: 102, 35: 105, 36: 108}
EXPECTED_REMAINING = {30: 18, 31: 15, 32: 12, 33: 9, 34: 6, 35: 3, 36: 0}
EXPECTED_RATINGS = {
...
def catalog_path(part: int) -> Path:
    return ROOT / f"data/catalog/sendai_challenge_project_reviews_part{part}.json"


def evidence_path(part: int) -> Path:
    return ROOT / f"data/evidence/sendai_challenge_project_reviews_part{part}_evidence.json"
...
def test_final_sendai_batches_form_exact_87_to_108_sequence():
    ...
    for part in PARTS:
        ...
        assert summary["cumulative_reviewed_record_count"] == EXPECTED_CUMULATIVE[part]
...
def test_final_sendai_batches_preserve_source_reported_ratings():
    for part in PARTS:
        ...
        assert [record["source_reported_evaluation"] for record in records] == EXPECTED_RATINGS[part]
        assert all(record["review_status"] == "reviewed_core_evaluation" for record in records)
        assert all("causal claim" in load(catalog_path(part))["quality_boundary"] for _ in [0])
...
def test_final_sendai_batches_have_one_to_one_valid_evidence():
    ...
    for part in PARTS:
        ...
        assert {packet["id"] for packet in packets} == {record["evidence_id"] for record in records}
        assert all(list(validator.iter_errors(packet)) == [] for packet in packets)
        assert all("独自" in packet["claims"][0]["review_note"] for packet in packets)


def test_sendai_manifest_closes_core_project_sequence_without_overclaiming_phase13():
    ...
    assert max(fact.get("cumulative_value", fact["value"]) for fact in batches) == 108
