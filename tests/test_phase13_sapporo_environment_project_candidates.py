...
def test_environment_historical_candidate_inventory_has_exact_74_rows():
    ...
    assert Counter(record["record_type"] for record in records) == {"main_project": 47, "other_project": 27}
...
def test_environment_candidates_preserve_representative_cost_anchors():
    ...
    assert records["komaoka_incineration_plant_renewal"]["planned_project_cost_yen"] == 46_197_000_000
...
def test_environment_final_review_promotes_all_74_rows_with_five_department_corrections():
    ...
    environment_denominator = next(field for field in denominators["field_denominators"] if field["field_id"] == "environment")

    assert environment_denominator["final_project_count"] == 74
    assert final["status"] == "final_field_direct_visual_review_complete_with_department_corrections"
...
def test_environment_exact_final_department_corrections_are_locked():
    ...
    assert corrections["yard_vehicle_electrification_study"]["final_department_ja"] == "経)中央卸売市場"
    assert corrections["combined_treatment_septic_tank_subsidy"]["final_department_ja"] == "環)環境事業部"
    assert corrections["water_facility_hydropower_installation"]["final_department_ja"] == "水)総務部"
    assert corrections["river_environment_improvement_for_life_and_nature"]["final_department_ja"] == "下)事業推進部"
...
def test_environment_final_review_is_reflected_in_global_metadata():
    ...
    environment = next(field for field in index["machizukuri_field_sources"] if field["field_id"] == "environment")
