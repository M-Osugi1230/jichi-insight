...
def test_measure10_linkage_counts_and_definition_boundaries():
    ...
    assert [record["series_id"] for record in records] == [
        f"policy-indicator-miyagi-{number}" for number in range(90, 99)
    ]
    assert {record["series_id"] for record in records if record["linkage_status"] == "linked"} == {
        "policy-indicator-miyagi-90",
        "policy-indicator-miyagi-92",
        "policy-indicator-miyagi-93",
        "policy-indicator-miyagi-94",
        "policy-indicator-miyagi-95",
        "policy-indicator-miyagi-96",
    }
    assert {
        record["series_id"] for record in records if record["linkage_status"] == "needs_review"
    } == {
        "policy-indicator-miyagi-91",
        "policy-indicator-miyagi-97",
        "policy-indicator-miyagi-98",
    }
...
def test_measure10_catalog_connection_statuses():
    ...
    assert {
        number for number, group in groups.items() if group["actual_linkage_status"] == "not_linked"
    } == {73, 79, 80}
