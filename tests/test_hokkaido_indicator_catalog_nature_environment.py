...
def test_catalog_schema_sequence_and_pages():
    ...
    assert (data["indicator_number_start"], data["indicator_number_end"]) == (99, 102)
    assert [item["indicator_number"] for item in data["items"]] == [99, 100, 101, 102]
...
def test_exact_values_periods_and_series_are_preserved():
    ...
    assert [[value["value"] for value in items[number]["series"][0]["values"]] for number in (99, 100, 102)] == [
        [100, 100, 100],
        [90.8, 100, 100],
        [105.0, 82.0, 82.0],
    ]
    assert [value["period"] for value in items[99]["series"][0]["values"]] == [
        "2021年",
        "2026年",
        "2031年",
    ]
    assert [value["period"] for value in items[102]["series"][0]["values"]] == [
        "2020年",
        "2025年",
        "2030年",
    ]
    assert [series["label"] for series in items[101]["series"]] == ["東部", "北部", "中部"]
    assert [series["values"][0]["value"] for series in items[101]["series"]] == [137, 121, 107]


def test_deer_range_targets_remain_conditional():
    item = next(item for item in load(CATALOG)["items"] if item["indicator_number"] == 101)
    east, north, central = item["series"]
    assert [value["value_text_original"] for value in east["values"][1:]] == ["37.5～50", "37.5～50"]
    assert [value["status"] for value in east["values"][1:]] == ["conditional", "conditional"]
...
def test_reference_breakdowns_and_publication_lags_are_separated():
    ...
    assert all(item["actual_linkage_status"] == "not_linked" for item in items.values())
    assert all(item["evaluation_status"] == "not_assessed" for item in items.values())
