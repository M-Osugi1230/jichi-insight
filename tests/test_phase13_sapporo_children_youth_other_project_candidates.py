...
CATALOG_PATH = ROOT / "data/catalog/sapporo_action_plan_children_youth_other_projects_candidates.json"
EVIDENCE_PATH = ROOT / "data/evidence/sapporo_action_plan_children_youth_other_projects_candidates_evidence.json"
...
def test_children_youth_other_candidate_inventory_preserves_goal_and_draft_page_counts():
    ...
    assert Counter(record["draft_page_label"] for record in records) == {52: 9, 53: 17, 59: 10, 63: 11}
...
def test_children_youth_other_candidate_inventory_preserves_representative_costs():
    ...
    assert records["private_nursery_development_subsidy"]["planned_project_cost_yen"] == 7_949_000_000
    assert records["second_child_guidance_center_development"]["planned_project_cost_yen"] == 3_451_000_000
    assert records["youth_science_museum_exhibit_zone_development"]["planned_project_cost_yen"] == 851_000_000
    assert records["reading_challenge_children_reading_promotion"]["planned_project_cost_yen"] == 31_000_000
...
def test_children_youth_final_denominator_and_final_review_are_both_exact_121():
    ...
    children = next(field for field in denominators["field_denominators"] if field["field_id"] == "children_youth")
