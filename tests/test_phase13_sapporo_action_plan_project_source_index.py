...
def test_sapporo_project_source_index_has_exact_official_document_spine():
    ...
    assert index["status"] == "source_documents_indexed_project_records_pending"
...
def test_sapporo_project_source_index_does_not_invent_project_allocation():
    ...
    assert summary["individual_project_records_reviewed"] == 0
...
def test_sapporo_project_source_index_keeps_review_boundary_explicit():
    ...
    assert "individual project" in boundary
...
def test_sapporo_manifest_records_document_index_without_city_completion():
    ...
    assert "個別事業" in source_index["interpretation_boundary"]
    assert any("資料10本" in item and "599事業" in item for item in manifest["remaining_work"])
