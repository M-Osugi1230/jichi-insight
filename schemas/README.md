...
47都道府県のように、件数そのものが完全性条件となる運用カタログは、架空fixtureではなく実カタログを専用テストで検証します。`prefecture_coverage.schema.json` は `data/catalog/prefecture_coverage.json` に直接適用し、47件の完全性、公式コード順、重複、地域区分、品質昇格をテストします。
...
| Prefecture coverage | 47都道府県の登録、公式入口、計画索引、Reviewed・公開状態 |
...
### Nationwide coverage status

- `registered`: 全国登録済み。公式URLは候補を含む
- `official_entry_verified`: 公式ホームページを手動確認
- `source_cataloged`: 総合計画等の公式入口を固定
- `reviewed_data`: 本文・数値・単位・期間を一次資料と照合
- `actuals_linked`: 年度実績と公式説明を接続
- `published`: 公開ゲートと本番Smokeを通過
