...
const roleLabels: Record<MiyagiIndicatorValue["role"], string> = {
  initial: "初期値",
  current: "現況値",
  intermediate_target: "中期末目標",
  final_target: "後期末目標",
};

function valueLabel(value: MiyagiIndicatorValue, unit: string) {
      ...
      <div className="pageShell">
        ...
        <section className={styles.progressSection} aria-labelledby="miyagi-review-progress">
          ...
          <div className={styles.progressDetail}>
            ...
            <p>
              後期末目標が「－」の{miyagiPolicyReviewStats.partiallySetTargets}件は、
              0ではなく未設定として保持しています。累計指標
              {miyagiPolicyReviewStats.cumulativeTargets}件は単年度値へ変換していません。
            </p>
          </div>
        </section>
        ...
        <section className="contentSection" id="indicators">
          ...
          <div className={styles.scopeStack}>
            {miyagiIndicatorScopes.map((scope) => (
              <section className={styles.scopeSection} key={scope.id}>
                <div className={styles.scopeHeader}>
                  ...
                  <strong>{scope.indicators.length}目標</strong>
                </div>

                <div className={styles.indicatorGrid}>
                  {scope.indicators.map((indicator) => (
                    <article className={styles.indicatorCard} key={indicator.id}>
                      <div className={styles.indicatorTop}>
                        <span>目標値No.{indicator.target_group_number}</span>
                        <StatusBadge
                          label={
                            indicator.target_setting_status === "set"
                              ? "後期目標あり"
                              : "後期目標未設定"
                          }
                          tone={
                      ...
                      <h4>{indicator.indicator_name_original}</h4>

                      {indicator.series.map((series, seriesIndex) => (
                        <div className={styles.seriesBlock} key={`${indicator.id}-${seriesIndex}`}>
                          {series.label ? <h5>{series.label}</h5> : null}
                      ...
                      <footer>
                        <span>指標No.{indicator.series_number}</span>
                        <a
                          href={`${implementationPlanUrl}#page=${indicator.source_pdf_page}`}
                          target="_blank"
                          rel="noreferrer"
                        >
                          PDF {indicator.source_pdf_page}ページ ↗
                        </a>
                      </footer>
                    </article>
                  ))}
