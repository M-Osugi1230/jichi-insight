...
export type Phase10UniformRecord = {
  ...
  name: string;
  region: string;
  status: "queued" | "source_indexing" | "linkage_in_progress" | "review_ready" | "complete";
  current_depth: Phase10UniformDepth;
  gap_count: number;
  next_gate: "source_inventory" | "annual_actuals_linkage" | "money_linkage" | "action_linkage" | "accountability_linkage" | "publication_verification";
  next_action: string;
};
...
export type Phase10Wave1Record = {
  ...
  next_gate: "source_inventory" | "annual_actuals_linkage" | "budget_linkage" | "project_spine" | "publication_verification";
  next_action: string;
};
...
export type Phase10Queue = {
  ...
  waves: Array<{ id: string; prefecture_codes: string[]; objective: string }>;
...
export type Phase10Uniformity = {
  ...
  status: "in_progress" | "verification_pending" | "complete";
  scope_version: string;
  dimensions: Array<{ id: Phase10DimensionId; label: string; completion_status: Phase10DepthStatus }>;
...
const statusRank: Record<Phase10DepthStatus, number> = { not_indexed: 0, indexed: 1, reviewed: 2, linked: 3 };
...
function loadCatalog<T>(filename: string): T {
  return JSON.parse(fs.readFileSync(path.join(findDataRoot(), "catalog", filename), "utf-8")) as T;
}
...
export function phase10UniformRecords(uniformity: Phase10Uniformity): Phase10UniformRecord[] {
  return prefectures.map(([prefecture_code, name, region]) => {
    const override = uniformity.overrides[prefecture_code];
    const current_depth = { ...uniformity.default_depth, ...(override?.current_depth ?? {}) };
    const gap_count = uniformity.dimensions.filter(
      (dimension) => statusRank[current_depth[dimension.id]] < statusRank[dimension.completion_status],
    ).length;
    return {
      prefecture_code,
      name,
      ...
      next_action: override?.next_action ?? uniformity.default_work.next_action_template.replace("{name}", name),
    };
  });
}
...
export function phase10UniformSummary(
  ...
  const summary = Object.fromEntries(
    uniformity.dimensions.map((dimension) => [dimension.id, { not_indexed: 0, indexed: 0, reviewed: 0, linked: 0 }]),
  ) as Record<Phase10DimensionId, Record<Phase10DepthStatus, number>>;
  for (const record of records) {
    for (const dimension of uniformity.dimensions) summary[dimension.id][record.current_depth[dimension.id]] += 1;
  }
  return { ...summary, prefecture_count: records.length, uniform_depth_complete: records.filter((record) => record.gap_count === 0).length };
}
...
export function phase10DepthLabel(status: Phase10DepthStatus): string {
  return { not_indexed: "未索引", indexed: "入口確認", reviewed: "Reviewed", linked: "目標へ接続" }[status];
}

export function phase10UniformRecordStatusLabel(status: Phase10UniformRecord["status"]): string {
  return { queued: "資料索引待ち", source_indexing: "資料索引中", linkage_in_progress: "接続作業中", review_ready: "公開検証待ち", complete: "同一粒度完了" }[status];
}
