...
export const hokkaidoIndicatorReviewStats = {
  ...
  comparabilityWarnings: hokkaidoReviewedIndicators.filter((indicator) =>
    indicator.comparability_note_original?.includes("単純比較"),
  ).length,
  notAvailableCurrentValues: hokkaidoReviewedIndicators.reduce(
    (total, indicator) =>
      total +
      indicator.series.filter(
        (series) => series.values[0]?.status === "not_available",
      ).length,
    0,
  ),
  boundedFinalTargets: hokkaidoReviewedIndicators.reduce(
    (total, indicator) =>
      total +
      indicator.series.filter(
        (series) => series.values[2]?.operator === "at_least",
      ).length,
    0,
  ),
