...
BOUNDARY = (
    ...
    "修正版、修正通知に現れる名称や数値が近似しても、明示的なReviewed crosswalkなしに"
    "同一系列へ結合しない。現況値、基準値、目標値、単年度実績、累計、参考値、全国値、"
    "予算・決算・事業費を推測分解せず、structured annual actualまたはfuture targetへ昇格しない。"
    "文字抽出ノイズ、欠損、複数系列、注記、改定前後の値は原文のまま保存し、政策達成、因果関係、"
    "自治体間比較、ランキングは判定しない。"
)
