...
| 1 | 北海道 | KPI本文Reviewed 29/108 | KPIカタログ化 | 指標30〜32（デジタル）を個票から抽出し、Evidence Packetを付与 |
...
- 指標1〜12（食）、13〜18（観光）、19〜29（ゼロカーボン）をReviewed
- KPI Evidence Packet：29件
- 指標13・14・16・24の複数系列を個別に保持
- 指標13〜16の「以上」を`at_least`として保持
- 指標14・15の現状値なしをnullで保持
- 指標16・17・24は複数政策分野への一意指標参照として保持
- 指標20の986→755→850という非単調な原文目標を修正せず保持
- 指標21・25の3月末時点値を年度値と混同しない
- 進捗・評価：未接続・未評価を維持

### Active

1. 指標30〜32（デジタル）の本文・値・単位・期間をReviewed化
2. 残る79指標を政策分野順に抽出
...
1. 残る79指標のReviewed化とEvidence Packet
2. 年度実績・進捗資料との接続
3. 自動テストと公開ゲート
4. 北海道公開ページ
