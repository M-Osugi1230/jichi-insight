...
| 1 | 北海道 | KPI本文Reviewed 91/108 | KPIカタログ化 | 指標92〜98（社会経済の基盤整備）を個票から抽出し、Evidence Packetを付与 |
...
- 指標1〜91を15政策分野にわたりReviewed
- KPI Evidence Packet：91件
- 指標13・14・16・24・37・50・54・56・58・59の複数系列を個別に保持
- 指標13〜16と指標84の「以上」を`at_least`として保持
...
- 指標46の「全国値」、指標52の「現状より増加」、指標80の「社会増」を条件目標として保持
- 指標49・53・61・80の公式0値を欠損補完と混同しない
- 指標50の男女別系列と2政策分野参照を保持
- 指標54の学校段階・男女別4系列を保持
- 指標56・58の小中2系列、指標59の小中高3系列を保持
- 指標55・56・58・59・90の100％目標を明示的な数値として保持
- 指標62の圏域別看護職員数、指標63の求人倍率をKPI系列へ混入しない
- 指標64・65・84〜87・89・91は公表年ではなく実績年・基準日を保持
...
- 指標74・75の固定閾値と過去5年平均の複合条件を保持
- 指標76の年齢別内訳、指標77の算出要素、指標78の全国推移、指標79の区域別病床をKPI系列へ混入しない
- 指標80の人口内訳、指標81の地域別隊員数、指標82の窓口別相談数をKPI系列へ混入しない
- 指標83を年度別署名数ではなく1965年以降の累計到達値として保持
- 指標84の管内別英語力、指標85の市区町村・在留資格・国籍別内訳をKPI系列へ混入しない
- 指標86〜89の3月末基準、指標90・91の4月1日基準を保持
- 指標88のデータなし年と指標90の比較不能期間を0や推計値で補完しない
- 指標89の地域別対策状況、指標90の病院数内訳、指標91の振興局別カバー率を全道KPI系列へ混入しない
- 進捗・評価：未接続・未評価を維持

### Active

1. 指標92〜98（社会経済の基盤整備）の本文・値・単位・期間をReviewed化
2. 残る17指標を政策分野順に抽出
...
1. 残る17指標のReviewed化とEvidence Packet
2. 年度実績・進捗資料との接続
3. 自動テストと公開ゲート
4. 北海道公開ページ
