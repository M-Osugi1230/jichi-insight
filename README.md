...
`47-prefecture registry / Wave 1 current plans 9 of 9 confirmed / Hokkaido indicators 1–108 reviewed with Evidence Packets / Fukuoka policy targets 1–118 reviewed / Miyagi 4 directions, 8 policies and 18 measures reviewed / 128 KPI groups and 149 series positioned / KPI groups 1–38 and series 1–40 reviewed / pre-alpha`
...
- 北海道の条件目標値：19件。前年比較、全国値、相対条件、法定雇用率、複合条件、社会増、範囲、過去最高値を数値化せず保持
- 北海道の複数系列：指標13・14・16・24・37・50・54・56・58・59・95・101
...
- 北海道の明示的ゼロ値：指標49、53、61、80の公式0値を欠損補完と区別
- 北海道の範囲目標：指標101の東部・北部・中部を3系列に分け、`37.5～50`と`25～50`を単一数値へ変換しない
- 北海道の過去最高値目標：指標107・108の「過去最高値」を条件目標として保持し、参考値60人・8個へ置換しない
...
- Reviewed KPI本文：38/128グループ、40/149系列（柱1、取組1〜5）
- KPI Evidence Packet：38件
- 後期末目標未設定：37系列。公式表の「－」を0へ変換しない
- 累計KPI：5件。公式名称の`［累計］`を単年度値へ変換しない
- Reviewed複数系列：グループ26の農業・水産業・林業を3系列のまま保持
- 単位記載なし：耕地利用率へ単位を推測で補完しない
- 原文数値表記：中期末目標の`21400`、`4126`へ桁区切りを追加しない
- 負値：経済成長率の-1.12ポイントを欠損・エラーへ変換しない
- 非単調目標：漁業産出額、水産加工品出荷額、木材・木製品出荷額、食料品製造業の製造品出荷額、所定外労働時間数、認定農業者数、スマートインターチェンジ等の利用台数を公式値のまま保持
- 残り：90目標グループ、109系列
...
- 公開済み総合評価：0件
- 次工程：宮城県の目標グループ39〜52・系列41〜57（柱2、取組6〜7）をEvidence Packet付きでReviewed化
